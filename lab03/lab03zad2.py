...
# fq=np.linspace(-1000,1000,2*4*fq+1, endpoint=True)
x3=[]

for f in fq:
   x3.append( np.sum( x_app * np.exp(-2j*np.pi*f*t_app)))
...
fq1_=np.linspace(-fq1,fq1,4*fq1+1, endpoint=True)
x3_1=[]
k=0
for f in fq1_:
   x3_1.append( np.sum( x_app * np.exp(-2j*np.pi*f*t_app)))
