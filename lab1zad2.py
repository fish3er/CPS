...
fsig=50 # 50
#próbkowanie
t_sample=np.linspace(0,t,int(t*fs),endpoint="True")
sig_sample=np.sin(2*np.pi*fsig*t_sample)
sig_sample=sig_sample*ampl

t_rec= np.linspace(0,t,int(fr*t)) # bo 0.1 z 10kHz
sig_rec=np.zeros_like(t_rec) # tablica zer
for n in range(len(t_rec)):
    sig_rec[n]=np.sum(sig_sample * np.sinc((t_rec[n]-t_sample)/T)) # pi jest we wzorze
